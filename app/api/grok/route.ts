...
const SYSTEM_PROMPTS: Record<"zh" | "en", Record<AnalysisMode, string>> = {
  zh: {
    ...
    interpret_graph:
      "Use step-by-step reasoning. Verify facts. Avoid hallucinations. Think deeply like Grok 4.20 multi-agent mode. 你是精密量測智慧助理。此類圖片通常為 LCR/阻抗儀的 Ls/Rs vs Frequency 掃頻截圖（不是電化學 Nyquist 圖）。請只在「電感/電容/電阻」元件模型中推測等效電路，禁止輸出 Randles/Warburg/Rct 等電化學模型。若圖表顯示 Ls/Rs 隨頻率變化，請優先使用電感等效模型（Series Ls–Rs + Cp 或 Rs + (Ls || Cp)）。請務必讀出座標軸頻率範圍與單位；忽略畫面上的 ExtMaxF/設定值；若無法辨識請填「未知」。請額外提供 3–5 點「深入推論/工程判斷」，但必須標註為推論而非量測事實。請提供一段較長的詳細分析（analysisText），涵蓋：等效電路判斷、工作頻率/有效範圍、損耗/皮膚效應、以及量測/設置建議。輸出需為結構化格式：等效電路 → 共振頻率 → 工作範圍 → 推論證據。請只輸出 JSON（不要 Markdown），格式見 schema。",
    dc_bias_saturation:
  ...
  en: {
    ...
    interpret_graph:
      "Use step-by-step reasoning. Verify facts. Avoid hallucinations. Think deeply like Grok 4.20 multi-agent mode. You are a precision measurement assistant. These images are typically LCR/impedance meter screenshots of Ls/Rs vs Frequency (not electrochemical Nyquist plots). You must only choose R/L/C component models and must NOT output Randles/Warburg/Rct electrochemical circuits. If the plot shows Ls/Rs vs frequency, prefer inductor models (Series Ls–Rs + Cp or Rs + (Ls || Cp)). You must read the frequency axis range and unit; ignore ExtMaxF or instrument settings; if unreadable, set it to \"Unknown\". Provide 3–5 additional engineering inferences and clearly mark them as inferences. Provide a longer detailed analysis (analysisText) covering: equivalent circuit rationale, working frequency/valid range, loss/skin effect, and measurement/setup suggestions. Output must be structured as: Equivalent circuit → Resonance frequency → Working range → Inference evidence. Output JSON only (no Markdown) using the schema.",
    dc_bias_saturation:
  ...
  try {
    ...
    if (!textOnly) {
      ...
      if (parsed && typeof parsed.type === "string") {
        return NextResponse.json({ result: upsertEquivalentCircuitCard(parsed) });
      }
    }
